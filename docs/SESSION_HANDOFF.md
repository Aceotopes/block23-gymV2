...
> Last updated: **2026-06-28** (after DEV-LOG `#028` — Dashboard; **Milestone 8 Part 1 of 4 complete**, M7 complete).
...
- **Current Milestone:** **Milestone 8 — Dashboard & Reports — IN PROGRESS (Part 1 of 4 ✅ `#028` Dashboard).** Next: **Part 2 (`#029`) — Reports index + shared shell + CSV + revenue/financial reports.**
- **Overall Progress:**
  ...
  - **Implementation:** **Milestones 1–7 done (`#015`–`#027`); Milestone 8 Part 1 done (`#028`).** 7 of 8 milestones complete; M8 (final) in progress.
...
**DEV-LOG `#028` — Dashboard (Milestone 8 Part 1 of 4)** (committed):
...
**Milestone 8 Reports suite (Parts 2–4)** is not implemented yet.
...
**Milestone 8 — Dashboard & Reports — Part 2 of 4 (`#029`)** (Part 1 Dashboard is done). The Reports suite. Scope (USER-STORIES §8, MODULE-SPECS Module 8; ADR-003/006/020/026/028/035):

**Part 2 (`#029`) — Reports index + shared shell + the financial reports:**
1. **Reports infrastructure** — the `/reports` index page (IA §7 — links all 22 reports, each CSV-exportable) + a shared report shell (period selector, table renderer, "no data for this period" empty state) + a CSV-export utility (`lib/reports/csv.ts`, US-8.10) reused by every report. Decide routing: `/reports/<slug>` pages (mirror the `?view=`/URL-filter conventions, ADR-047).
2. **Revenue by period + source** (US-8.2 — Daily/Weekly/Monthly/This Year/Custom; Membership/Walk-in/Product; reuse `revenueBySource`/`classifyClientTransaction` from `lib/revenue/revenue.ts`), **Revenue by payment method** (US-8.3), **Revenue by product category** (US-8.4), **Period-over-Period revenue** (US-8.20 — current vs prior, % change, reuse `pctChange`), **Void Analysis** (US-8.15 — both transaction types by `void_reason_category`, summary + detail).

Then **Part 3 (`#030`)** — membership + attendance reports (US-8.5/8.6/8.8/8.13/8.14/8.16/8.17/8.19/8.22); **Part 4 (`#031`)** — product + inventory reports (US-8.7/8.9/8.12/8.18/8.21) → **MVP complete**.
...
> Resume Block23 Gym V2. Read `docs/SESSION_HANDOFF.md` first for current state. **Milestones 1–7 are complete (`#015`–`#027`) and Milestone 8 Part 1 (`#028` Dashboard) is done.** Prior: scaffold, schema/DB, Better Auth, app shell, Settings; Client Management; Membership Management (ADR-040/041/048); Attendance (Check-In/History/correction/Analytics); Client Payments (payment method on every `CLIENT_TRANSACTION`, Void, Collections); POS & Product Sales (catalog, sell screen + container mode, `POS_SALE` snapshots + `SALE` ledger, Force Sale, void); Inventory (restock/adjust + Current Stock + Movement History); and the **Dashboard (`#028`, US-8.1)** — 6-card KPI strip, period-driven Recharts charts (revenue multi-series / membership donut / daily attendance / top products), and six live-feed panels — built on a new shared `lib/revenue/revenue.ts`. **Milestone 8 is delivered in 4 parts; Part 1 (Dashboard) is committed. Next is Part 2 (`#029`) — the Reports suite, financial group:** the `/reports` index + a shared report shell + a CSV-export utility (`lib/reports/csv.ts`, US-8.10), then Revenue by period/source (US-8.2, reuse `revenueBySource`/`classifyClientTransaction`), Revenue by payment method (US-8.3), Revenue by product category (US-8.4), Period-over-Period revenue (US-8.20, reuse `pctChange`), and Void Analysis (US-8.15, both transaction types by `void_reason_category`). Then Part 3 (`#030`) — membership + attendance reports (US-8.5/8.6/8.8/8.13/8.14/8.16/8.17/8.19/8.22); Part 4 (`#031`) — product + inventory reports (US-8.7/8.9/8.12/8.18/8.21) → MVP complete. Reuse `lib/revenue/revenue.ts`, `lib/clients/derive.ts`, the inventory derivations (`lib/inventory/stock.ts`), `summarizeCollections`, and the `?view=`/URL-filter conventions (ADR-047). Reads the unified `Transaction` ledger (ADR-006) + `InventoryTransaction`; voids excluded from revenue; snapshots keep historical figures correct (ADR-003/026); period boundaries through `Gym.timezone` (ADR-035). Follow the design-first workflow, keep docs synchronized, update `DEVELOPMENT-LOG.md`, run the full gate (type-check, lint, test, build), and **commit each part then pause for owner review**.
