...
- **Current Milestone:** **Milestone 8 — Dashboard & Reports — ✅ COMPLETE (all 4 parts: `#028` Dashboard · `#029` Reports index + financial reports · `#030` membership + attendance/clients reports · `#031` product + inventory reports).** **The MVP is complete — all 8 milestones delivered.** Next work is Phase 2 (post-MVP backlog) or owner-directed.
- **Overall Progress:**
  ...
  - **Implementation:** **All 8 milestones done (`#015`–`#031`).** Milestone 8 (final) delivered in 4 parts (`#028`–`#031`) — MVP complete.
...
- **Key decision:** "units/servings sold" and COGS are taken from the **SALE inventory ledger** (stock units), not the line-item `quantity` — container-mode sales record `quantity` in containers but move `containers × servings_per_container` stock, so the ledger is the consistent cross-mode measure (and matches Inventory Usage). Slow-moving uses a dedicated 30/60/90 window selector (per the AC) rather than the shared period presets.
- All reuse the shared `ReportPeriodSelector` / `ReportSegmentFilter` / `CsvExportButton` / `reportRange`, plus `lib/reports/products.ts`. Reads the unified `Transaction`/`TransactionLineItem` ledger (ADR-006) joined to `InventoryTransaction` (ADR-004); voids excluded (COMPLETED POS_SALE only); cost snapshots keep gross profit correct (ADR-026); FORCED_SALE markers/void reversals never count as shrinkage (ADR-034); period bounds via `Gym.timezone` (ADR-035); URL state (ADR-047).
- Verified: type-check ✓ · lint ✓ · test 133/133 ✓ · build ✓ (`/reports/[slug]` 2.15 kB) · **Neon query-shape smoke** (all 5 report query shapes executed against the live schema; script removed).
...
- **Reports — products + inventory (`#031`, US-8.7/8.9/8.12/8.18/8.21 — Milestone 8 Part 4)** — the final five reports: Best sellers, Gross profit, Inventory usage (+ shrinkage), Restock cost, Slow-moving / dead stock. New pure lib `lib/reports/products.ts` (+9 tests). Each filterable (period + category, or the 30/60/90 window for slow-moving) and CSV-exportable. Units/COGS read from the SALE inventory ledger so container-mode sales count consistently. **Milestone 8 and the MVP are complete — all 19 reports under `/reports` are live.**
...
**The MVP is complete — all 8 milestones (`#015`–`#031`) delivered.** There is no next MVP milestone. Future work draws from the **Phase 2 — Post-MVP backlog** (ROADMAP.md): the P2 user stories (e.g. US-8.11 PDF export, plus the deferred Clients/Membership/Attendance/Payments P2 items) and the multi-tenant SaaS expansion path (keep `gym_id` everywhere; do not build multi-tenancy features until in scope). Cross-cutting items still open: **Playwright E2E** is deferred (now that check-in/membership/payments/POS/inventory all ship, a combined E2E pass is a sensible first post-MVP task), and the **sample owner credentials must be changed before real use** (Known Issue #6).

**`#031` is staged for commit — the owner controls when to commit.** After committing, pause for owner review per the agreed M8 per-part cadence.
...
> Resume Block23 Gym V2. Read `docs/SESSION_HANDOFF.md` first for current state. **The MVP is complete — all 8 milestones delivered (`#015`–`#031`).** Milestone 8 (Dashboard & Reports) shipped in 4 parts: `#028` Dashboard (US-8.1), `#029` Reports index + 5 financial reports, `#030` the 9 membership + attendance/clients reports, and `#031` the 5 product + inventory reports (US-8.7/8.9/8.12/8.18/8.21 — on the new pure lib `lib/reports/products.ts`). All 19 reports live under `/reports`. `#031` is verified (type-check ✓ · lint ✓ · test 133/133 ✓ · build ✓) and **staged for commit — the owner controls when to commit; pause for owner review after.** No MVP work remains; future work is the **Phase 2 post-MVP backlog** (ROADMAP.md) — e.g. Playwright E2E (now that all flows ship), US-8.11 PDF export, the other P2 stories, and the multi-tenant SaaS path (keep `gym_id` everywhere; don't build multi-tenancy until in scope). Before real use, change the sample owner credentials (Known Issue #6). Follow the design-first workflow and keep docs synchronized for any further change.
