...
### Milestone 8 — Dashboard & Reports ✅ COMPLETE (Part 1 `#028` Dashboard · Part 2 `#029` financial reports · Part 3 `#030` membership + attendance reports · Part 4 `#031` product + inventory reports — **MVP COMPLETE**)
...
- [x] Revenue reports by period (Daily / Weekly / Monthly / This Year / Custom Range) and source (membership / walk-in / product) (US-8.2) — **Part 2 (`#029`)**
- [x] Revenue by payment method (Cash, GCash, Card, Other) (US-8.3) — **Part 2 (`#029`)**
- [x] Revenue by product category (US-8.4) — **Part 2 (`#029`)**
- [x] Attendance reports: period selector, member vs. walk-in breakdown, unique vs. total check-ins, period-over-period comparison toggle, CSV export (US-8.5) — **Part 3 (`#030`)**
- [x] Membership reports: active, expired, expiring-soon lists (US-8.6) — **Part 3 (`#030`)**
- [x] Best-selling products report by units/servings and by revenue; slow-moving sort option (ascending) (US-8.7) — **Part 4 (`#031`)**
- [x] Frequent walk-in clients report (high visits, low membership conversion) (US-8.8) — **Part 3 (`#030`)**
- [x] Inventory usage report with shrinkage derivation: movements per product over date range, adjustment breakdown by reason category (US-8.9) — **Part 4 (`#031`)**
- [x] Gross profit report: revenue, COGS (cost_price_snapshot × quantity), gross profit, and margin % per product; blended totals; flags null-cost-snapshot sales (US-8.12) — **Part 4 (`#031`)**
- [x] CSV export for all reports (US-8.10) — **Part 2 (`#029`)**: shared `lib/reports/csv.ts` + `CsvExportButton`, wired into every report as it ships
- [x] Member engagement report: active members ranked by visit frequency, least engaged first (US-8.13) — **Part 3 (`#030`)**
- [x] At-risk members report: active members not attending within threshold, ranked by days since last visit (US-8.14) — **Part 3 (`#030`)**
- [x] Void analysis report: voided transactions by void_reason_category and period, spanning both transaction types; summary + detail sections (US-8.15) — **Part 2 (`#029`)**
- [x] New vs. renewals report: new memberships (renewed_from_membership_id IS NULL) vs. renewals per period, with renewal rate %; filterable by plan (US-8.16) — **Part 3 (`#030`)**
- [x] Membership plan performance report: per plan — count, revenue, average price paid, plan status (US-8.17) — **Part 3 (`#030`)**
- [x] Restock cost report: period-total inventory spend from InventoryTransaction.total_restock_cost, per product and grand total; null entries excluded with count noted (US-8.18) — **Part 4 (`#031`)**
- [x] Membership net change report: new + renewals − expired per month, running cumulative active count; last 12 months default (US-8.19) — **Part 3 (`#030`)**
- [x] Period-over-period revenue comparison: current vs. prior period side-by-side by source, with % change; This Week/Month/Year presets + custom (US-8.20) — **Part 2 (`#029`)**
- [x] Slow-moving / dead stock report: active products with zero sales in 30/60/90-day window; cost value locked in stock; longest-inactive first (US-8.21) — **Part 4 (`#031`)**
- [x] Converted walk-ins report: clients who converted from walk-in to member in the period (ADR-020 derivation), with conversion timeline and pre-conversion visit count (US-8.22) — **Part 3 (`#030`)**

> **Milestone 8 is delivered in four parts.** **Part 1 (`#028`) — Dashboard (US-8.1) ✅:** the 6-card KPI strip, the period-driven chart row (revenue multi-series line, membership donut, daily attendance stacked bar, top products horizontal bar — Recharts), and six live-feed panels (recent POS sales, expiring soon, inventory alerts w/ stockout estimate, Today's Collections, frequent walk-ins, at-risk members). Reuses central client derivation, the inventory derivations (`lib/inventory/stock.ts`), `summarizeCollections`, and a new shared revenue-aggregation lib (`lib/revenue/revenue.ts`). **Part 2 (`#029`) ✅** — Reports index + shared report shell + CSV export (US-8.10) + the revenue/financial reports (US-8.2/8.3/8.4/8.20/8.15): the `/reports` index (grouped, links each report) + `/reports/[slug]` dispatch, the shared period selector (`lib/reports/period.ts`) + CSV utility (`lib/reports/csv.ts` + `CsvExportButton`), and the five financial reports reusing `lib/revenue/revenue.ts` + `summarizeCollections`. **Part 3 (`#030`) ✅** — the nine membership + attendance/clients reports (US-8.5/8.6/8.8/8.13/8.14/8.16/8.17/8.19/8.22), each a `/reports/[slug]` component reusing the shared period selector + CSV export + `lib/clients/derive.ts`, on new pure libs `lib/reports/{membership,attendance,conversion}.ts`. **Part 4 (`#031`) ✅** — the five product + inventory reports (US-8.7 Best sellers / US-8.9 Inventory usage + shrinkage / US-8.12 Gross profit / US-8.18 Restock cost / US-8.21 Slow-moving), each a `/reports/[slug]` component reusing the shared period selector + category/window filters + CSV export, on a new pure lib `lib/reports/products.ts`; reads the `Transaction`/`TransactionLineItem` ledger joined to the `InventoryTransaction` ledger (units/COGS from the SALE ledger so container sales count consistently). **Milestone 8 and the MVP are complete — all 8 milestones (`#015`–`#031`) delivered.**
