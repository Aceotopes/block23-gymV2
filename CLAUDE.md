...
**MVP complete — all 8 milestones delivered (`#015`–`#031`).** Planning, the tech stack, and the design language are stable. Done: Next.js 15 scaffold + design tokens; Neon PostgreSQL with Prisma 7 (11-entity schema, migrations); Better Auth owner login (username + password, server-managed sessions, default-deny route protection); the 8-entry app shell; the Settings module; Client Management (centralized derivation, list/profile/archive, ADR-047 list-state-in-URL); Membership Management (create/renew/cancel with canonical date math, ad-hoc custom durations, and the Membership Plan catalog in Settings — US-3.1/3.2/3.3/3.9/3.10, ADR-040/041/048); Attendance (three-view module per ADR-023 — Check-In Station with branch flows + Today's list, Attendance History with URL filters, same-day `time_in` correction, and Attendance Analytics with Recharts trend/day-of-week/by-hour + insight/alert panels — US-4.1–4.5/4.8/4.9/4.10/4.11); Client Payments (the `CLIENT_TRANSACTION` + payment-method layer — M3 membership create/renew and M4 walk-in check-in create the transaction + line item atomically; Payment History with URL filters; additive Void with required `void_reason_category`; End-of-Day Collections by method spanning both transaction types — US-5.1/5.2/5.3/5.4, ADR-006/024/028); and POS & Product Sales (POS `?view=` shell — product catalog CRUD + archive/restore, STANDARD vs SERVING_BASED with conditional fields, live gross margin, category management; the sell screen with category tabs/grid/search/cart + container mode; checkout with payment method + cash-change; `POS_SALE` creation with price/cost snapshots + `SALE` inventory entries decrementing `current_stock`; Force Sale → `FORCED_SALE` marker; POS History + additive void — US-6.1–6.10/6.13/6.14/6.15/6.16, ADR-003/004/009/011/026/027/034); and Inventory (the Inventory `?view=` shell — restock as a `PURCHASE` ledger entry raising `current_stock` with optional `total_restock_cost`; manual adjustment as a signed `ADJUSTMENT` with a required owner reason category — `FORCED_SALE` excluded per ADR-034 — note for `OTHER`, below-zero decrease blocked; the Current Stock view with low-stock/reorder flags, remaining-servings, days-until-stockout, this-month shrinkage amber/red, and an inventory-valuation footer — all derived at query time; and the per-product Movement History ledger — US-7.1/7.2/7.4/7.5/7.6/7.7/7.8, ADR-004/026/028/034); and the Dashboard (Milestone 8 Part 1 — US-8.1: a 6-card KPI strip, a Today/Week/Month period selector driving four Recharts charts (revenue multi-series, membership donut, daily attendance, top products), and six live-feed panels — recent POS sales, expiring soon, inventory alerts with stockout estimates, Today's Collections, frequent walk-ins, at-risk members — on a new shared `lib/revenue/revenue.ts`, reusing central derivation + the inventory derivations + `summarizeCollections`). The Collections summary (M5) already queries both transaction types, so POS revenue surfaces there automatically. The Reports module (Milestone 8 Part 2) adds the `/reports` index + `/reports/[slug]` dispatch, a shared period selector + CSV-export utility (`lib/reports/`), and the five financial reports (Revenue by period/source, by payment method, by category, Period-over-Period, Void Analysis — US-8.2/8.3/8.4/8.20/8.15/8.10) reusing `lib/revenue/revenue.ts` + `summarizeCollections`. Reports Part 3 adds the nine membership + attendance/clients reports (Membership status lists, New vs. renewals, Plan performance, Net change; Attendance report with a period-over-period toggle, Member engagement, At-risk members, Frequent walk-ins, Converted walk-ins — US-8.5/8.6/8.8/8.13/8.14/8.16/8.17/8.19/8.22) on new pure libs `lib/reports/{membership,attendance,conversion}.ts`, with `ReportProps` extended to carry the gym derivation `thresholds` and a generic URL `ReportSegmentFilter`; all reuse `lib/clients/derive.ts` (ADR-002/019/020/040/041). Reports Part 4 (`#031`) adds the final five product + inventory reports (Best sellers — US-8.7, Gross profit — US-8.12, Inventory usage + shrinkage — US-8.9, Restock cost — US-8.18, Slow-moving / dead stock — US-8.21) on a new pure lib `lib/reports/products.ts`, reading the unified `Transaction`/`TransactionLineItem` ledger joined to the `InventoryTransaction` ledger — units/COGS taken from the SALE ledger so container-mode sales count consistently (ADR-003/004/006/026/034). **Milestone 8 (Dashboard & Reports) is the final MVP milestone, delivered in four parts — all four are done, completing the MVP.** **`docs/SESSION_HANDOFF.md` is the live status + resume point — read it first.** See `docs/TECH-STACK.md` for the approved stack and development rules, `docs/DESIGN-SYSTEM.md` for tokens/components/patterns, and `docs/INFORMATION-ARCHITECTURE.md` for the navigation/screen map.
...
These ADRs (in `DECISIONS.md`, currently ADR-001 through ADR-049; ADR-030–032 intentionally unused) represent resolved questions. Reopening one requires a new ADR with an explicit rejected-alternative and reasoning. Treat conflicts with these decisions as signals that a proposed change needs more analysis, not as reasons to silently override them.
...
**Design language (ADR-045 → superseded by ADR-049):** The shipped language is the **"Block 23 Console"** (Claude Design) direction — dark-only, warm near-black canvas (`#100F0D`), **violet primary `#8B43F0` for action/active state only (never a status color)**, semantic emerald/amber/red-rose/sky (at-risk = orange to stay distinct from amber) kept distinct from the primary; **Space Grotesk (display) + IBM Plex Sans (body) + IBM Plex Mono (tabular money/numbers + uppercase micro-eyebrows)**; shadcn `new-york` / lucide icons; tokens are `--b23-*` CSS variables (SSOT `docs/DESIGN-TOKENS.MD`) with shadcn's semantic tokens remapped onto them in `globals.css`; `:root` is the single source of truth (`.dark` inherits). Status badges always carry a text label + icon — never color alone. ADR-045's indigo/slate + Geist palette is superseded; its structural principles stand. Narrative spec in `DESIGN-SYSTEM.md` (being refreshed to ADR-049).
