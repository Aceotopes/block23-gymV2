...
**Implementation in progress — Milestones 1–5 complete and Milestone 6 Part 1 done; next is Milestone 6 Part 2 (POS sell screen + checkout + stock + history/void).** Planning, the tech stack, and the design language are stable. Done: Next.js 15 scaffold + design tokens; Neon PostgreSQL with Prisma 7 (11-entity schema, migrations); Better Auth owner login (username + password, server-managed sessions, default-deny route protection); the 8-entry app shell; the Settings module; Client Management (centralized derivation, list/profile/archive, ADR-047 list-state-in-URL); Membership Management (create/renew/cancel with canonical date math, ad-hoc custom durations, and the Membership Plan catalog in Settings — US-3.1/3.2/3.3/3.9/3.10, ADR-040/041/048); Attendance (three-view module per ADR-023 — Check-In Station with branch flows + Today's list, Attendance History with URL filters, same-day `time_in` correction, and Attendance Analytics with Recharts trend/day-of-week/by-hour + insight/alert panels — US-4.1–4.5/4.8/4.9/4.10/4.11); Client Payments (the `CLIENT_TRANSACTION` + payment-method layer — M3 membership create/renew and M4 walk-in check-in create the transaction + line item atomically; Payment History with URL filters; additive Void with required `void_reason_category`; End-of-Day Collections by method spanning both transaction types — US-5.1/5.2/5.3/5.4, ADR-006/024/028); and the Product Catalog (POS `?view=` shell — product CRUD + archive/restore, STANDARD vs SERVING_BASED with conditional fields, live gross margin, category management — US-6.1–6.5/6.15, ADR-004/005). The POS sell screen, checkout, stock deduction (inventory ledger), container-mode sale, Force Sale, and POS History/void are Milestone 6 Part 2; `current_stock` is ledger-driven and starts at 0 (restock is Milestone 7). The Collections summary already queries both transaction types, so POS revenue surfaces there automatically once sales exist. **`docs/SESSION_HANDOFF.md` is the live status + resume point — read it first.** See `docs/TECH-STACK.md` for the approved stack and development rules, `docs/DESIGN-SYSTEM.md` for tokens/components/patterns, and `docs/INFORMATION-ARCHITECTURE.md` for the navigation/screen map.
