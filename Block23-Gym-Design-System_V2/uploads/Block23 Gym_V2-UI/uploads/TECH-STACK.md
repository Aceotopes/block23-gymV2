...
**Credentials-only at MVP.** Login is by **username + password** via Better Auth's username plugin + email/password credential provider (US-1.1). Public sign-up is disabled; the owner is provisioned by the seed (`prisma/seed.ts`). No OAuth providers at MVP. Social login (Google, etc.) is a post-MVP addition that Better Auth supports without an architecture change.

**Better Auth uses the domain `User` table — no parallel user table (ADR-043, ADR-046).** Better Auth is configured against the existing Prisma `User` model; `gym_id` and `role` are declared as Better Auth **additional fields**. The `User` carries Better Auth's core fields (`name`, `email`, `email_verified`, `image`) and the username plugin's `username`/`display_username`. **Credential password hashes live in the Better-Auth-owned `account` table — not on `User` (ADR-046).** Better Auth owns three tables: `session`, `account`, `verification` (the documented exception to `gym_id`-on-every-entity — auth infra, tenancy via `User.gym_id`). The session payload exposes `{ userId, gymId, role }`. The Prisma `User` model remains the single canonical user record — do not create a second Better-Auth-owned user table.
